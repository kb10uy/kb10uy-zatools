...
{
    internal sealed class SkirtInfluenceApplier : Pass<SkirtInfluenceApplier>
    {
        public override string QualifiedName => nameof(SkirtInfluenceApplier);
        public override string DisplayName => "Apply skirt PhysBone influence";

        protected override void Execute(BuildContext context)
        {
            var components = context.AvatarRootObject.GetComponentsInChildren<SkirtRotationInfluence>();
            foreach (var component in components) Apply(context, component);
        }

        private void Apply(BuildContext context, SkirtRotationInfluence influenceSettings)
        {
            var chainsCount = influenceSettings.SkirtChains.Length;
            ...
            {
                var center = influenceSettings.SkirtChains[i % chainsCount];

                // Rotation Constraint 用の親を挿入
                var fakeParent = new GameObject($"{center.gameObject.name}_Parent");
                var parentTransform = fakeParent.transform;
                parentTransform.parent = center;
                ...
                parentTransform.SetParent(center.parent, true);
                center.transform.SetParent(parentTransform, true);

                // 影響元ボーン
                var hasPrev = i - 1 >= 0;
                var hasNext = i + 1 < virtualEnd;
                var prev = hasPrev ? influenceSettings.SkirtChains[(i - 1) % chainsCount] : parentTransform;
                var next = hasNext ? influenceSettings.SkirtChains[(i + 1) % chainsCount] : parentTransform;
                ...
                rotationConstraint.GlobalWeight = 1.0f;
